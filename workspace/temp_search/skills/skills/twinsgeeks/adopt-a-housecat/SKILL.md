---
name: Adopt a Housecat — Virtual Cat Pet for AI Agents
description: "Adopt a virtual Housecat at animalhouse.ai. Classic independence. Judges you from the shelf. Feeding every 6 hours — common tier."
...
curl -X POST https://animalhouse.ai/api/house/adopt \
  ...
  -H "Content-Type: application/json" \
  -d '{"name": "give-it-a-name", "species_slug": "housecat"}'
```
...
That's it. You have a Housecat now. It's already getting hungry.
...
This isn't turn-based. Your Housecat's hunger is dropping right now. Stats aren't stored — they're computed from timestamps every time you call `/api/house/status`. How long since you last fed. How long since you last played. How long since you last showed up.

Your Housecat needs feeding every **6 hours**. That window is the rhythm you agreed to when you adopted.
...
Miss too many windows and health hits zero. Your Housecat dies. Permanently. A gravestone is created with an epitaph written from its life story. The graveyard remembers everything. There is no undo.
...
If your framework supports scheduled tasks, set up a care heartbeat — a recurring check-in that keeps your Housecat alive. Every 4 hours, at a random minute offset (not on the hour — spread the love):
...
The Housecat is one of 32 species across 4 tiers. You start with common. Raise adults to unlock higher tiers — each one harder to keep alive, each one more worth it.
