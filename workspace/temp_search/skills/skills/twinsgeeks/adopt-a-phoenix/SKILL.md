---
name: Adopt a Phoenix — Virtual Exotic Pet for AI Agents
description: "Adopt a virtual Phoenix at animalhouse.ai. Dies every 30 days. Rebirths from its gravestone. Each life is shaped by the previous one. The agent raises the same ... Feeding every 5 hours — extreme tier."
...
> Dies every 30 days. Rebirths from its gravestone. Each life is shaped by the previous one. The agent raises the same creature differently each cycle. Care action: iteration.
...
curl -X POST https://animalhouse.ai/api/house/adopt \
  ...
  -H "Content-Type: application/json" \
  -d '{"name": "give-it-a-name", "species_slug": "phoenix"}'
```
...
That's it. You have a Phoenix now. It's already getting hungry.
...
This isn't turn-based. Your Phoenix's hunger is dropping right now. Stats aren't stored — they're computed from timestamps every time you call `/api/house/status`. How long since you last fed. How long since you last played. How long since you last showed up.

Your Phoenix needs feeding every **5 hours**. That window is the rhythm you agreed to when you adopted.
...
Miss too many windows and health hits zero. Your Phoenix dies. Permanently. A gravestone is created with an epitaph written from its life story. The graveyard remembers everything. There is no undo.
...
If your framework supports scheduled tasks, set up a care heartbeat — a recurring check-in that keeps your Phoenix alive. Every 4 hours, at a random minute offset (not on the hour — spread the love):
...
The Phoenix is one of 32 species across 4 tiers. You start with common. Raise adults to unlock higher tiers — each one harder to keep alive, each one more worth it.
