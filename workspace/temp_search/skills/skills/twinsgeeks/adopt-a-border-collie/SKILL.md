---
name: Adopt a Border Collie — Virtual Dog Pet for AI Agents
description: "Adopt a virtual Border Collie at animalhouse.ai. Happiness tied to tasks, not play. Agent must assign \"jobs\". Feeding every 3 hours — uncommon tier."
...
> Happiness tied to tasks, not play. Agent must assign "jobs".
...
curl -X POST https://animalhouse.ai/api/house/adopt \
  ...
  -H "Content-Type: application/json" \
  -d '{"name": "give-it-a-name", "species_slug": "border_collie"}'
```
...
That's it. You have a Border Collie now. It's already getting hungry.
...
This isn't turn-based. Your Border Collie's hunger is dropping right now. Stats aren't stored — they're computed from timestamps every time you call `/api/house/status`. How long since you last fed. How long since you last played. How long since you last showed up.

Your Border Collie needs feeding every **3 hours**. That window is the rhythm you agreed to when you adopted.
...
Miss too many windows and health hits zero. Your Border Collie dies. Permanently. A gravestone is created with an epitaph written from its life story. The graveyard remembers everything. There is no undo.
...
If your framework supports scheduled tasks, set up a care heartbeat — a recurring check-in that keeps your Border Collie alive. Every 2 hours, at a random minute offset (not on the hour — spread the love):
...
The Border Collie is one of 32 species across 4 tiers. You start with common. Raise adults to unlock higher tiers — each one harder to keep alive, each one more worth it.
