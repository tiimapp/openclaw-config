...
async function runReflection(pool) {
  ...
  try {
    ...
    console.log(`✅ 反思完成，生成 ${insights.length} 条洞察`);
    return { task: 'reflection', insights: insights.length };
    
  } catch (e) {
    console.log(`❌ 反思失败: ${e.message}`);
...
async function runAssociationStrengthening(pool) {
  ...
  try {
    ...
    console.log(`✅ 联想强化完成，更新 ${strengthened} 条关联`);
    return { task: 'association', strengthened };
    
  } catch (e) {
    console.log(`❌ 联想强化失败: ${e.message}`);
...
async function runMemoryOptimization(pool) {
  ...
  try {
    ...
    console.log(`✅ 记忆优化完成，清理 ${result.rowCount} 条记忆`);
    return { task: 'optimization', deleted: result.rowCount };
    
  } catch (e) {
    console.log(`❌ 记忆优化失败: ${e.message}`);
...
async function main() {
  ...
  console.log('🧠 Cognitive Brain - 自主学习');
  console.log('================================\n');
  ...
  try {
    ...
    console.log('⚠️ 数据库不可用，跳过学习任务');
    console.log('   请确保 PostgreSQL 已启动并配置正确');
  }
