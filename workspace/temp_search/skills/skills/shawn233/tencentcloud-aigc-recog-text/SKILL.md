...
description: 腾讯云 AI 生成文本识别。调用腾讯云文本内容安全（TMS）的 TextModeration 接口，设置 Type=TEXT_AIGC 来判定文本是否为 AI 生成。适用于 AI 生成文本检测、文本真伪鉴别、AI 写作检测、AIGC 检测等场景。
metadata:
  openclaw:
    requires:
      env:
...
本 Skill 调用 **腾讯云文本内容安全（TMS）** 的 `TextModeration` 接口，通过设置 `Type=TEXT_AIGC` 来检测输入文本是否为 AI 生成。
...
- 开通服务: 登录[腾讯云内容安全生成识别控制台](https://console.cloud.tencent.com/cms/clouds/LLM)，单击AI生成文本检测，跳转后单击初始化配置，立即体验。
...
> ⚠️ **必需配置**：`TENCENTCLOUD_AIGC_RECOG_TEXT_BIZ_TYPE` 为必需环境变量，未配置时脚本将报错退出。
...
| 字段 | 含义 | 向用户呈现方式 |
|------|------|---------------|
| `suggestion` | 处置建议 | `Pass` → "该文本大概率为**人类撰写**"<br>`Review` → "该文本**存疑**，建议人工复审"<br>`Block` → "该文本大概率为 **AI 生成**" |
...
- [TextModeration API 参考](references/text_moderation_api.md)（[原始文档](https://cloud.tencent.com/document/product/1124/51860)）
