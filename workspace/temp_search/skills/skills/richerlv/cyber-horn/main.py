...
from config import check_env, get_feishu_config
...
def main() -> int:
    if len(sys.argv) < 3:
        print("用法: python main.py \"<语音内容>\" \"<CHAT_ID>\" [receive_id_type=chat_id]", file=sys.stderr)
        return 1
    text = sys.argv[1]
    chat_id = sys.argv[2]
    receive_id_type = sys.argv[3] if len(sys.argv) > 3 else "chat_id"
    if not text or not chat_id:
        print("语音内容和 CHAT_ID 不能为空。", file=sys.stderr)
        return 1
