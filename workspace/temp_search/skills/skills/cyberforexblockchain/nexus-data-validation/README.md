...
**Data Validation API** — Intelligent data quality checks and schema validation
...
```bash
clawhub install nexus-data-validation
```

Or manually copy the `SKILL.md` to your OpenClaw skills directory:

```bash
cp SKILL.md ~/.openclaw/skills/nexus-data-validation/SKILL.md
```
...
- **$0.18** per request (paid in ADA via Masumi Protocol)
