...
import { ApiRoutes, ApiV1SkillResponseSchema, ApiV1SkillVersionListResponseSchema, ApiV1SkillVersionResponseSchema, } from '../../schema/index.js';
...
export async function cmdInspect(opts, slug, options = {}) {
    ...
    try {
        const skillResult = await apiRequest(registry, { method: 'GET', path: `${ApiRoutes.skills}/${encodeURIComponent(trimmed)}`, token }, ApiV1SkillResponseSchema);
        ...
        if (options.files || options.file || options.version || options.tag) {
            ...
            versionResult = await apiRequest(registry, {
                method: 'GET',
                path: `${ApiRoutes.skills}/${encodeURIComponent(trimmed)}/versions/${encodeURIComponent(targetVersion)}`,
                token,
