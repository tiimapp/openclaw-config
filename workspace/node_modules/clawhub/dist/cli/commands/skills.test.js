...
describe('cmdUninstall', () => {
    ...
    it('rejects unsafe slugs', async () => {
        await expect(cmdUninstall(makeOpts(), '../evil', { yes: true }, false)).rejects.toThrow(/invalid slug/i);
        await expect(cmdUninstall(makeOpts(), 'demo/evil', { yes: true }, false)).rejects.toThrow(/invalid slug/i);
    });
    it('fails when skill is not installed', async () => {
        vi.mocked(readLockfile).mockResolvedValue({ version: 1, skills: {} });
        await expect(cmdUninstall(makeOpts(), 'missing', {}, false)).rejects.toThrow('Not installed: missing');
    });
    ...
    it('updates lockfile after removing directory', async () => {
        ...
        expect(rmMock.mock.invocationCallOrder[0]).toBeLessThan(writeLockfileMock.mock.invocationCallOrder[0]);
    });
