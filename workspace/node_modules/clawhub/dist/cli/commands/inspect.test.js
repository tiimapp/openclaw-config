...
vi.mock('../../http.js', () => ({
    ...
    fetchText: (...args) => mockFetchText(...args),
}));
...
describe('cmdInspect', () => {
    it('fetches latest version files when --files is set', async () => {
        ...
        expect(secondArgs?.path).toBe(`${ApiRoutes.skills}/${encodeURIComponent('demo')}/versions/${encodeURIComponent('1.2.3')}`);
    });
    ...
    it('rejects when both version and tag are provided', async () => {
        await expect(cmdInspect(makeOpts(), 'demo', { version: '1.0.0', tag: 'latest' })).rejects.toThrow('Use either --version or --tag');
    });
});
